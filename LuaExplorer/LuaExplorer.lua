--[[
  "Ultimate" Lua Explorer
  by Buck Pyland

  Based on Lua Explorer "Advanced" by John Doe

  Original comments for Lua Explorer "Advanced"
  ...
  Original comments for Lua Explorer
...
local function makeMenuItems(obj)
  ...
  --
  -- Buck changed to work around cases where:
  -- 1. obj has an __index metamethod that displays an error message
  --    if a field does not exist. An example is the ctxdata.config table
  --    which is created by the context package.
  -- 2. There is no properties field in the table.
  --
  ...
  if not (success and props) then
    ...
    -- far.Message(props, "Error in __index metamethod", nil, "wl")
  end
...
local function luaexp_prompt(Title, Prompt, Src)
  repeat
    local expr = far.InputBox(nil, Title:gsub('&', '&&', 1), Prompt, Title, Src,
                              nil, nil, far.Flags.FIB_ENABLEEMPTY)
...
local function editValue(obj, key, title, del)
  if del then
    if 1 == far.Message(('%s is a %s, do you want to remove it?'):format(valfmt(key), type(obj[key]):upper()), 'REMOVE: ' .. title, ';YesNo', 'w') then
      obj[key] = nil
    end
  else
...
local function showLocals(level, shift)
  ...
  if not info then
    -- mf may not be available in certain cases, eg. LuaFAR for Editor
    local mf = mf or nil
...
brkeys = {
    ...
    },
    {
      BreakKey = 'Shift+F3',
      action = function(obj, key, kpath)
                 if jit and key ~= nil and type(obj[key]) == 'function' then
